// dsh-ssh static Cordis plugin (agent-preset plane).
//
// Provides the `ssh.hosts` service (isolated per preset mount) and the
// ssh_host_list / ssh_host_add / ssh_host_update / ssh_host_remove /
// ssh_bash model tools. Host inventory persists in ~/.dsh/ssh-hosts.json.
// Password auth uses SSH_ASKPASS (no pty, no sshpass) so it works inside
// the harness command sandbox.

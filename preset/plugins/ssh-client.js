...
`;
		...
		function Section() {
			...
			txChildren.push(e('div', { key: 'tr1', className: 'ssh-row' },
				...
				e(Input, { value: txLocal, onInput: function (ev) { setTxLocal(ev.target.value) }, onChange: function (ev) { setTxLocal(ev.target.value) }, style: { flex: 1, minWidth: 180 }, placeholder: txDir === 'upload' ? '本地文件路径，如 /tmp/a.bin' : '本地目录或完整路径，如 ~/ 或 /tmp/a.bin' }),
				e(Input, { value: txRemote, onInput: function (ev) { setTxRemote(ev.target.value) }, onChange: function (ev) { setTxRemote(ev.target.value) }, style: { flex: 1, minWidth: 180 }, placeholder: txDir === 'upload' ? '远端目标路径（以 / 结尾=目录，自动拼接文件名）' : '远端文件路径，如 /etc/config.conf' }),
